...
async def get_calendar_event(body: GetCalendarEventInput, graph: GraphDep):
    data = await graph.get_calendar_event(body.meeting_id)
...
async def get_recording_status(body: GetRecordingStatusInput, graph: GraphDep):
    enabled = await graph.get_recording_status(body.meeting_id)
    return GetRecordingStatusOutput(meeting_id=body.meeting_id, recording_enabled=enabled)
...
async def post_adaptive_card(body: PostAdaptiveCardInput, graph: GraphDep):
    await graph.post_adaptive_card(body.meeting_id, body.card_payload, body.target_participant_ids)
